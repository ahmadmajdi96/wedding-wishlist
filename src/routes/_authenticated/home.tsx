...
const ICONS: Record<string, any> = {
  ...
  ShieldCheck, Headphones, Crown, Star, Calendar,
};
...
const contentOpts = queryOptions({ queryKey: ["home-content"], queryFn: () => getHomeContent() });
...
export const Route = createFileRoute("/_authenticated/home")({
  loader: ({ context }) =>
    Promise.all([
      ...
      context.queryClient.ensureQueryData(contentOpts),
    ]),
  component: HomePage,
});
...
function HomePage() {
  ...
  const { data: content } = useSuspenseQuery(contentOpts);
  const [slide, setSlide] = useState(0);

  const slides = content.slides;
  const S = (key: string) => content.sections[key];
  const show = (key: string) => Boolean(S(key)?.is_visible);
    ...
    <Phone>
        ...
        <div className="px-5 animate-slide-up stagger-1">
          <div className="relative rounded-[28px] overflow-hidden h-52 shadow-gold">
            {slides.map((s: any, i: number) => (
              <div key={s.id} className="absolute inset-0 transition-opacity duration-700" style={{ opacity: slide === i ? 1 : 0 }}>
                <img src={s.image_url} className="absolute inset-0 w-full h-full object-cover" alt={s.title} />
                ...
                <div className="absolute inset-0 p-5 flex flex-col justify-end text-white">
                  ...
                  {s.cta_label && s.cta_slug && (
                    ...
                    >
                      {s.cta_label} <ChevronLeft className="size-3.5" />
                    </Link>
                  )}
                </div>
              </div>
      ...
      {show("categories") && (
        <div className="px-5 mt-6 animate-slide-up stagger-2">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-display font-bold text-base">{S("categories").title}</h3>
            <Link to="/categories" className="text-[11px] text-[color:var(--color-primary)] font-bold flex items-center gap-0.5">
              {S("categories").cta_label || "عرض الكل"} <ChevronLeft className="size-3" />
            </Link>
          </div>
      ...
      {show("promo") && (
        <div className="px-5 mt-6 animate-slide-up stagger-3">
          <div className="relative rounded-3xl overflow-hidden gradient-navy p-5 text-white">
            {S("promo").image_url && (
              <img src={S("promo").image_url} alt="" className="absolute inset-0 w-full h-full object-cover opacity-30" />
            )}
            ...
            <div className="relative flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <p className="gold-text font-display font-bold text-xs tracking-wider">{S("promo").subtitle}</p>
                <h3 className="font-display font-bold text-lg mt-1 leading-tight">{S("promo").title}</h3>
                {S("promo").cta_label && (
                  <Link
                    to="/categories/$slug"
                    params={{ slug: S("promo").cta_slug || "halls" }}
                    className="mt-3 bg-white text-[color:var(--color-navy)] text-xs font-bold rounded-full px-4 py-2 inline-flex items-center gap-1"
                  >
                    {S("promo").cta_label} <ChevronLeft className="size-3.5" />
                  </Link>
                )}
              </div>
      ...
      {show("featured") && (
        <div className="mt-6 animate-slide-up stagger-4">
          <div className="flex items-center justify-between px-5 mb-3">
            <div>
              <h3 className="font-display font-bold text-base">{S("featured").title}</h3>
              <p className="text-[11px] text-muted-foreground">{S("featured").subtitle}</p>
            </div>
            <Link to="/categories" className="text-[11px] text-[color:var(--color-primary)] font-bold flex items-center gap-0.5">
              {S("featured").cta_label || "الكل"} <ChevronLeft className="size-3" />
            </Link>
          </div>
      ...
      {show("popular") && (
        <div className="px-5 mt-6 animate-slide-up stagger-5">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-display font-bold text-base">{S("popular").title}</h3>
            <Link to="/categories" className="text-[11px] text-[color:var(--color-primary)] font-bold flex items-center gap-0.5">
              {S("popular").cta_label || "عرض الكل"} <ChevronLeft className="size-3" />
            </Link>
          </div>
          ...
          <Link to="/plan" className="block">
            <div className="relative gradient-cream rounded-3xl p-5 overflow-hidden border border-[color:var(--color-border)]/60">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <p className="text-[11px] gold-text font-bold tracking-wider">{S("plan").subtitle}</p>
                  <h3 className="font-display font-bold text-lg mt-1">{S("plan").title}</h3>
                  <p className="text-xs text-muted-foreground mt-1">{S("plan").body}</p>
                </div>
      ...
      {show("new") && (
        <div className="mt-6 animate-slide-up">
          <div className="flex items-center justify-between px-5 mb-3">
            <h3 className="font-display font-bold text-base">{S("new").title}</h3>
            <span className="text-[10px] bg-[color:var(--color-soft-rose)] text-[color:var(--color-primary)] font-bold rounded-full px-2 py-1">جديد</span>
          </div>
          <div className="flex gap-3 overflow-x-auto px-5 pb-2 hide-scrollbar snap-x">
      ...
      {show("testimonials") && content.testimonials.length > 0 && (
        <div className="px-5 mt-6 animate-slide-up">
          <h3 className="font-display font-bold text-base mb-3">{S("testimonials").title}</h3>
          <div className="flex gap-3 overflow-x-auto pb-2 hide-scrollbar snap-x">
            {content.testimonials.map((t: any) => (
              <div key={t.id} className="app-section rounded-3xl p-4 shrink-0 w-[290px] snap-start">
                <div className="flex items-center gap-3">
                  {t.avatar_url && <img src={t.avatar_url} alt="" className="size-12 rounded-full object-cover" />}
                ...
                <p className="text-sm text-muted-foreground leading-relaxed mt-3">{t.quote}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Trust strip */}
      {show("trust") && content.features.length > 0 && (
        <div className="px-5 mt-6 mb-2 grid grid-cols-3 gap-2 animate-slide-up">
          {content.features.map((f: any) => {
            ...
            return (
              <div key={f.id} className="app-pill rounded-2xl p-3 text-center">
                <Icon className="size-5 text-[color:var(--color-primary)] mx-auto" />
                <p className="text-[10px] font-bold mt-1.5">{f.label}</p>
              </div>
            );
          })}
        </div>
